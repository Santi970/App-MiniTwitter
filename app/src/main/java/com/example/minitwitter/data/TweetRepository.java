...
public class TweetRepository {
    ...
    public MutableLiveData getAllTweets() {
        if(allTweets == null){
    ...
    public MutableLiveData getFavsTweets() {
        if (favTweets == null){
    ...
    public void createTweet (String mensaje){
        ...
        Call<Tweet> call =  authTwitterService.createTweets(requestCreateTweet);
    ...
    public void deleteTweet(final int idTweet){
        ...
        call.enqueue(new Callback<TweetDeleted>() {
            ...
            public void onResponse(Call<TweetDeleted> call, Response<TweetDeleted> response) {
                if (response.isSuccessful()){
                    List<Tweet> cloedTweets = new ArrayList<>();
                    for (int i=0; i<allTweets.getValue().size(); i++ ){
                        if (allTweets.getValue().get(i).getId()!= idTweet){
                               cloedTweets.add(new Tweet(allTweets.getValue().get(i)));
                        }
                    }

                    allTweets.setValue(cloedTweets);
                    getFavsTweets();
